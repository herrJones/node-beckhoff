...
/**
 * Modify the remote port in a commandheader
 * used for specific system commands
 * 
 * @param {Object} options 
 * @param {Object} settings 
 * @param {integer} port 
 * @returns 
 */
function prepareCommandHeaderCustom(options, settings, port) {
  const tmpHeader = prepareCommandHeader(options, settings);

  // overwrite the standard configured port
  tmpHeader.writeUInt16LE(port, 12);
...
function prepareCommandRead(options, settings, kind) {
  ...
  switch (kind) {
    case 'getvalue' :
      msgData.writeUInt32LE(options.symbols[0].group  || config.ADS_IDXGRP.RW_SYMVAL_BYHANDLE, 0);
...
function prepareCommandWrite(options, settings, kind) {
  ...
  switch (kind) {
    case 'setvalue':
      ...
      symData = config.createPlcValue(options.symbols[0]);
...
function prepareCommandWriteControl(options, settings) {
  ...
  msgData.writeUInt16LE(config.getValueFromName(config.ADS_STATE, options.symbols.adsState), 0);
...
function prepareCommandAddNotification(options, settings) {
  ...
  //msgData.writeUInt32LE(options.symbols[0].group,   0);
  //msgData.writeUInt32LE(options.symbols[0].offset,  4);
  msgData.writeUInt32LE(config.ADS_IDXGRP.RW_SYMVAL_BYHANDLE,  0);
...
function prepareCommandDelNotification(options, settings) {
  ...
  msgData.writeUInt32LE(options.symbols[0].notify, 0);
  ...
  switch (kind) {
    case 'getvalue':
      for (let i=0; i<options.symbols.length; i++) {
        ...
        msgData.writeUInt32LE(options.symbols[i].group  || config.ADS_IDXGRP.RW_SYMVAL_BYHANDLE, offset + 0);
    ...
    case 'setvalue':
      ...
      //offset -= 12;
      for (let i=0; i<options.symbols.length; i++) {
        const value = config.createPlcValue(options.symbols[i]);
...
function prepareGetHandleRequest(options) {
  ...
  if (options.symbols.length == 1) {
    ...
    options.request.group   = config.ADS_IDXGRP.GET_SYMHANDLE_BYNAME;
    ...
    options.request.group   = config.ADS_IDXGRP.SUMUP_READWRITE;
    ...
    for (let i = 0; i < options.symbols.length; i++) {
      ...
      rwBuffer.writeUInt32LE(config.ADS_IDXGRP.GET_SYMHANDLE_BYNAME, index + 0);
...
function prepareDelHandleRequest(options) {
  ...
  if (options.symbols.length == 1) {
    ...
    options.request.group  = config.ADS_IDXGRP.RELEASE_SYMHANDLE;
    options.request.offset = 0;
    ...
    // add request data to the header
    let offset = 0;
    for (let i = 0; i < options.symbols.length; i++) {
      
      rwBuffer.writeUInt32LE(config.ADS_IDXGRP.RELEASE_SYMHANDLE, offset + 0);
