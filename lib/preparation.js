...
function preparePlcRead(options, settings) { 
  let msgData = Buffer.alloc(12 * options.request.length).fill(0);
  let result = null;
  ...
  if (options.request[0].hasOwnProperty("handle")) {
    ...
    for (let i=0; i<options.request.length; i++) {
      ...
      msgData.writeUInt32LE(options.request[i].idxGroup || ADSIGRP.RW_SYMVAL_BYHANDLE, offset + 0);
      //msgData.writeUInt32LE(options.request[i].group || ADSIGRP.RW_SYMVAL_BYHANDLE, offset + 0);
...
function preparePlcSymbolHandle(symbols, options, settings) {
  ...
  for (let i = 0; i < options.request.length; i++) {
    ...
    if (element.handle == -1) {
      toFetch.push(element.name);
    }
  }

  let rwBuffer = null;
  let rwData = null;
  switch (toFetch.length) {
    case 0:
      result = null;
      break;
    case 1:
      rwBuffer = Buffer.from(toFetch[0]);
      rwData = {
        cmd     : ADSCMD.ReadWrite,
      ...
      break;
    default:
      let rwLength = toFetch.length * 16;
      ...
      for (let i=0; i<toFetch.length; i++) {
        let index = 16*i;
        let symName = Buffer.from(toFetch[i]);
        
        rwBuffer.writeInt32LE(ADSIGRP.GET_SYMHANDLE_BYNAME, index + 0);
        rwBuffer.writeInt32LE(0x00000000, index + 4);
        rwBuffer.writeInt32LE(0x00000004, index + 8);
        rwBuffer.writeInt32LE(symName.length + 1, index + 12);

        let symBufLen = symNames.length + symName.length + 1;
      ...
      rwData = {
        ...
        invoke  : options.invoke,
        request : {
          idxGroup  : ADSIGRP.SUMUP_READWRITE,
          idxOffset : toFetch.length,
          rLength   : toFetch.length * 16,
          wLength   : rwBuffer.length, 
          buffer    : rwBuffer
        }
      }
...
function createPlcValue(symbol) {
  let result = Buffer.alloc(symbol.size).fill(0);
