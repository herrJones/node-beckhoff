...
function preparePlcRead(options, settings) { 
  ...
  let offset = 0;
  msgData.writeUInt32LE(options.request[0].group,  offset + 0);
  msgData.writeUInt32LE(options.request[0].offset, offset + 4);
  ...
  if (options.request[0].hasOwnProperty('handle')) {
    ...
    for (let i=0; i<options.request.length; i++) {
      ...
      msgData.writeUInt32LE(options.request[i].group  || config.ADSIGRP.RW_SYMVAL_BYHANDLE, offset + 0);
      msgData.writeUInt32LE(options.request[i].offset || options.request[i].handle , offset + 4);
  ...
  if (options.request.length == 1) {
    ...
    const tmpOptions = {
      ...
      request : {
        group   : config.ADSIGRP.SUMUP_READ,
        offset  : options.request.length,
        rLength : symLen,
...
function preparePlcWrite(options, settings) {
  ...
  if (options.request[0].handle != -1) {
    ...
    msgData.writeUInt32LE(options.request[0].group || config.ADSIGRP.RW_SYMVAL_BYHANDLE, 0);
  ...
  /* ----- NEW NEW NEW ---- 
  // are we dealing with symbol write request?
  if (options.request[0].hasOwnProperty('handle')) {
  ...
  switch (options.request.length) {
    ...
    case 1:
      ...
      rwData = {
        ...
        invoke  : options.invoke,
        request : {
          group   : config.ADSIGRP.GET_SYMHANDLE_BYNAME,
          offset  : 0x00000000,
          rLength : 0x00000004,
    ...
    default:
      ...
      rwData = {
        ...
        invoke  : options.invoke,
        request : {
          group   : config.ADSIGRP.SUMUP_READWRITE,
          offset  : options.request.length,
...
function preparePlcHandleRelease(options, settings) {
  ...
  if (options.request.length == 1) {
    ...
    for (let i = 0; i < options.request.length; i++) {
      ...
      msgData.writeUInt32LE(options.request[i].group,  offset +  0);
      msgData.writeUInt32LE(options.request[i].offset, offset +  4);
...
function prepareAddPlcNotification(options, settings) {
...
function prepareDelPlcNotification(options, settings) {
...
module.exports = {
  ...
  prepareAddPlcNotification,
  prepareDelPlcNotification
};