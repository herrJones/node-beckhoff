...
class beckhoffBridge extends events {
  ...
  initSocket() {
    ...
    this.sock.on('data', (data) => {
      ...
      if (this.develop.verbose) {
        debugVerbose(config.getTimestamp() + ' - RX : len = ' + (this.sock.bytesRead - this.rxOffset));
      }
  ...
  checkRxData(rxlen) {
    ...
    if (this.develop.debug) {
      debugVerbose('BKHF RX  : ' + result.toString('hex') + '\n');
    }
  ...
  async sendBuffer (txdata, kind) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      this.sock.write(txdata, (err) => {
        ...
        if (this.develop.verbose) {
          debugRaw('TX : %i bytes sent - %s', this.sock.bytesWritten, kind);
        }
