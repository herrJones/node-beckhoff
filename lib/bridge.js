...
class beckhoffBridge extends events {
  ...
  track_notification(options) {
    return new Promise((resolve, reject) => {
      this.db.run(`
        insert into tracking(time, invokeId, options, kind)
          values (datetime('now'), ?, ?, 'notify')
        `, [options.header.invoke, JSON.stringify(options)], (err => {
        if (err) {
          reject(err);
        } else {
          resolve('OK');
  ...
  async db_store_notifyhistory(symbols) {
    const result = [];
    const selStmt = 'select handle, name, kind from symbols where notify = ?';
    ...
    return new Promise((resolve, reject) => {
      symbols.forEach((symbol) => {
        ...
        this.db.get(selStmt, [symbol.notify], (err, row) => {
          if (err) {
            reject(err.message);
          }
          ...
          try {
            ...
            newSymbol.value = after.analyzePlcValue(newSymbol, symbol.data);

            result.push(newSymbol);

            this.db.run(insStmt, [row.name, newSymbol.value], (err) => {
              if (err) {
                reject(err.message);
              }
            ...
            console.log('notify error: ' + exc.message);
            updCnt++; 
          }
        });
      });

      while (updCnt == symbols.length) {
        config.sleep(1);
      }
      resolve(result);
    });
