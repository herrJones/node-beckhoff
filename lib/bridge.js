...
class beckhoffBridge extends events {
  ...
  async db_store_notifyhistory(symbols) {
    ...
    //debugVerbose('db_store_notifyhistory: ' + JSON.stringify(symbols));
    ...
    return new Promise((resolve, reject) => {
      this.db.all(selStmt, notifies, (err, rows) => {
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          debugVerbose('db_store_notifyhistory: ' + JSON.stringify(symbol));
          const newSymbol = {
            timestamp : symbol.timestamp,
            name  : rows[i].name,
            kind  : rows[i].kind,
            value : after.analyzePlcSymbolValues(symbol.data, rows[i])
          };
