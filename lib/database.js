...
class BeckhoffDB {
  ...
  create_database () {
    this.#database.parallelize(() => {
      ...
      `, [], (err) => {
        ...
        this.#database.run(`
          CREATE UNIQUE INDEX IF NOT EXISTS idx_symbol_grp_ofs_name
              ON symbols(idxGroup, idxOffset, name asc)
        `);
        this.#database.run(`
        ...
        this.#database.run(`
          CREATE VIEW IF NOT EXISTS vw_rpcmethods AS
            select s.idxGroup, s.idxOffset, 
                   ...
                   r.parm_in, r.parm_out, r.adstype as returntype, r.retsize as returnsize,
                   coalesce((select handle from SYMBOLS where name = s.name || '#' || r.name), 0) as handle,
                   coalesce((select count(1) from SYMBOLS where name = s.name || '#' || r.name), 0) as created
              from RPCMETHODS r,
                   DATATYPES d,
                   SYMBOLS s
                      ...
                      if (err) {
                        ...
                        element.rpccalls.forEach(rpcCall => {
                          this.#database.run(rpcStmt, [
                            ...
                            JSON.stringify(rpcCall.parm_in), JSON.stringify(rpcCall.parm_out)]);
                        });
  ...
  /**
   * 
   * @param {array} symbols to fetch info for 
   * @returns {Promise} array of data fetched 
   */
  ...
  symbolinfo_fetchrpcmethod (symbols) {
    ...
    for (let i = 0; i < symbols.length; i++) {
      if (i > 0) {
        sqlQry += ' union select * from VW_RPCMETHODS';
      }
      sqlQry += ' where name = "' + symbols[i].name.toUpperCase() +'"';
      if (symbols[i].method != null) {
        sqlQry += ' and method = "' + symbols[i].method.toUpperCase() + '"';
      }
    }
      ...
      this.#database.all(sqlQry, [], (err, rows) => {
        ...
        for (let r = 0; r < rows.length; r++) {
          ...
          const rpcInfo = {
            ...
            'handle'  : rows[r].handle,
            'parm_in' : JSON.parse(rows[r].parm_in),
            'parm_out': JSON.parse(rows[r].parm_out),
            'created' : rows[r].created
          };
  ...
  symbolinfo_store_rpcmethod(symbols) {
    let selStmt =
      `select idxGroup, idxOffset, name
         from SYMBOLS
        where NAME = ?`;
    let insStmt =
      `insert or ignore into SYMBOLS
           (idxGroup, idxOffset, name, kind, comment)
        values (?, ?, ?, 'method', 'rpc method')`;

    const selParms = [];
    if (symbols.length == 1) {
      selParms.push(symbols[0].name);
    } else {
      selStmt = selStmt.replace('= ?', 'in (?)');

      for (let s = 0; s < symbols.length; s++) {
        selParms.push(symbols[s].name);

        selStmt = selStmt.replace('?)', '?,?)');
      }
      selStmt = selStmt.replace('?,?)', '?)');
    }
    ...
    return new Promise((resolve, reject) => {
      try {
        this.#database.all(selStmt, selParms, (err, rows) => {
          if (err) {
            reject(err);
            return;
          }
          ...
          for(let r = 0; r < rows.length; r++) {
            const s = symbols.findIndex(obj => obj.name.toUpperCase() === rows[r].name);

            this.#database.run(insStmt, [rows[r].idxGroup, rows[r].idxOffset, rows[r].name + '#' + symbols[s].method]);
          }
