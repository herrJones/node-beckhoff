...
function analyzePlcSymbols(data) {
  ...
  function analyzeSymbol(pos) {
    ...
    symbol.flags = config.ADS_DATA_TYPE_FLAGS.toStringArray(symbol.flags);
    ...
    if (symbol.flags.includes('Attributes')) {
      const attrCnt = data.readUInt16LE(pos);
      pos += 2;

      for (let i = 0; i < attrCnt; i++) {
        const curAttr = {
          name  : data.readUInt8(pos + 0),
          value : data.readUInt8(pos + 1)
        }
