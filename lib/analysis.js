...
function analyzePlcSymbols(data) {
  ...
  function analyzeSymbol(pos) {
    const symbol = {
      ...
      //arritems: data.readUInt16LE(pos + 30) + 1,
      handle  : -1,
      notify  : -1
    };
...
function analyzePlcDataTypes(data) {
  ...
  while (typePos < data.length) {
    ...
    let value = data.toString('binary', tmpPos, config.getStringEnd(tmpPos, data.slice(tmpPos, tmpPos + curType.name)));
    tmpPos += curType.name;
    curType.name = value.toUpperCase();

    value = data.toString('binary', tmpPos, config.getStringEnd(tmpPos, data.slice(tmpPos, tmpPos + curType.kind)));
    tmpPos += curType.kind;
    curType.kind = value;

    value = data.toString('binary', tmpPos, config.getStringEnd(tmpPos, data.slice(tmpPos, tmpPos + curType.comment)));
