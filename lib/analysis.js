...
function analyzePlcDataTypes(data) {
  ...
  function analyzeRpcMethod(pos) {
    ...
    for (let i = 0; i < method.parmsize; i++) {
      ...
      if (parm.flags[0] == 'In') {
        tmp_prm.id = parm_in_id++;
        method.parm_in.push(tmp_prm);
      } else {
        tmp_prm.id = parm_out_id++;
        method.parm_out.push(tmp_prm);
      }
...
function analyzePlcRpcMethodCall(data, symbols) {
  ...
  for (let p = 1; p <= symbols[0].parm_out.length; p++) {
    const refParm = symbols[0].parm_out.find(obj => obj.id == p);
    const tmpSymbol = {
      'kind' : refParm.kind
    };
    refParm.value = config.analyzePlcValue(tmpSymbol, data.slice(dataOffset, dataOffset + refParm.size));

    dataOffset += refParm.size;
  }
