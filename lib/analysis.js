...
function analyzePlcSymbols(data) {
  ...
  while (symPos < data.length) {
    ...
    const curSym = {
      //symPos : symPos,
      //curLen : curLen,
      idxGroup  : data.readUInt32LE(symPos + 4),
      idxOffset : data.readUInt32LE(symPos + 8),
      size      : data.readUInt32LE(symPos + 12),
      name      : data.readUInt16LE(symPos + 24) + 1,
      kind      : data.readUInt16LE(symPos + 26) + 1,
      comment   : data.readUInt16LE(symPos + 28) + 1,
      handle    : -1
    };

    let tmpPos = symPos + 30;
    let value = data.toString('binary', tmpPos, getStringEnd(tmpPos, data.slice(tmpPos, tmpPos + curSym.name)));
    tmpPos += curSym.name;
    curSym.name = value.toUpperCase();

    value = data.toString('binary', tmpPos, getStringEnd(tmpPos, data.slice(tmpPos, tmpPos + curSym.kind)));
    tmpPos += curSym.kind;
    curSym.kind = value;

    value = data.toString('binary', tmpPos, getStringEnd(tmpPos, data.slice(tmpPos, tmpPos + curSym.comment)));
...
function analyzePlcWrite(data, symbols) {
  ...
  const result = {
    error  : getNameFromValue(config.ERRORS, data.readUInt32LE(0))
  };
