...
BeckhoffClient.prototype.fetchPlcSymbolHandle = function(options, txData) {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      //console.log('len = ' + symbols.length + ' - ' + JSON.stringify(symbols));
      
      
      after.analyzePlcSymbolHandles(Buffer.from(data.buffer), options.request);
...
BeckhoffClient.prototype.readPlcData = async function(symData, callback) {
  ...
  try {
    ...
    const rxData = await this.plc_invoke(options.invoke, txData, 'read');
    console.log(JSON.stringify(rxData));
    rxInfo = after.analyzePlcRead(Buffer.from(rxData.buffer), options.request)
...
BeckhoffClient.prototype.writePlcData = async function (symData, callback) {
  ...
  try {
    ...
    rxInfo = await this.plc_invoke(options.invoke, txData, 'write');
