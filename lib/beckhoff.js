...
class BeckhoffClient extends events {
  ...
  track_PlcInvoke_Start(invokeId) {
    const handle = setTimeout((invokeId) => {
      this.#db.run(`
      update tracking
         set handle = -1,
             data = 'timeout'
  ...
  track_PlcInvoke_Clear(invokeId) {
    this.#db.get('select data from tracking where invokeId = ?', [invokeId], (err, row) => {
      this.#db.run('delete from tracking where invokeId =  ?', [invokeId]);
  ...
  recv_Plc_Data (data) {
    ...
    switch (result.header.command) {
      ...
      case config.ADSCMD.ReadDeviceInfo :
        ...
        this.track_PlcInvoke_Resolve(result.header.invoke, result);
        // if (this.#settings.develop.verbose) 
        //const test = this;
        //  console.log('plcInfo: ' + JSON.stringify(result));
        break;
      ...
      case config.ADSCMD.ReadState :
        ...
        if (reference.settings.develop.verbose) 
          console.log('plcState: ' + JSON.stringify(result));
        break;
...
BeckhoffClient.prototype.getPlcInfo = async function(callback) {
  ...
  try {
    this.track_PlcInvoke_Start(options.invoke);
    await this.plc.sendBuffer(txHeader, 'info');

    while (this.track_PlcInvoke_Check(options.invoke) != -1) {
      await config.sleep(5);
    }
    rxInfo = this.track_PlcInvoke_Clear(options.invoke)
  }
  catch (exc) {
    console.log(exc);
  }
...
BeckhoffClient.prototype.getPlcState = async function(callback) {
  ...
  try {
    let data = this.plc.sendBuffer(txHeader, 'state');
