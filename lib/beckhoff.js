...
class BeckhoffClient extends events {
  ...
  create() {
    ...
    if (this.#settings.develop.save) {
      if (!this.#settings.develop.location || (this.#settings.develop.location == undefined)) {
        sqliteConnection = __dirname + '/beckhoff.db3';
      } else {
        sqliteConnection = this.#settings.develop.location + '/beckhoff.db3';
      } 
    }
  ...
  cleanup_history(settings) {
    ...
    this.#db.cleanup_database(settings);
  }
  ...
  */
  ...
  db_fetch_symbolhandle (symbols) {
    ...
    return new Promise((resolve, reject) => {
      this.#db.all(symQry, names, async (err, rows) => {
        if (err) {
          reject(err);
          return;
        }
    ...
    return new Promise((resolve, reject) => {
      this.#db.get(`select ? as name, 
        ...
        if (row.num > 0) {
          ...
          this.#db.run(`insert into symbols (handle, name) 
  ...
  */
  ...
  async plc_fetch_symbolhandles (symbols) {
    ...
    const options = {
      ...
      symbols : await this.db.symbolinfo_fetch(symbols)
    };
  ...
  async recv_Plc_Data (data) {
    ...
    if (result.header.error != 0) {
      ...
      resolved = await this.track_PlcInvoke_Resolve(result.header.invoke, result)
        .catch((error) => {
          debugError('recv_Plc_Data: ' + error);
        });
      return;
    }
    switch (result.header.command) {
      ...
      case config.ADS_CMD.Read :
        rxData = await this.track_PlcInvoke_GetRequest(result.header.invoke)
          .catch((error) => {
            debugError('recv_Plc_Data (cmd: Read): ' + error);
          });
      ...
      case config.ADS_CMD.Write :
        rxData = await this.track_PlcInvoke_GetRequest(result.header.invoke)
          .catch((error) => {
            debugError('recv_Plc_Data (cmd: Write): ' + error);
          });
      ...
      case config.ADS_CMD.NotificationAdd :
        rxData = await this.track_PlcInvoke_GetRequest(result.header.invoke)
          .catch((error) => {
            debugError('recv_Plc_Data (cmd: NotificationAdd): ' + error);
          });
      ...
      case config.ADS_CMD.NotificationDel :
        rxData = await this.track_PlcInvoke_GetRequest(result.header.invoke)
          .catch((error) => {
            debugError('recv_Plc_Data (cmd: NotificationDel): ' + error);
          });
      ...
      case config.ADS_CMD.Notification :      
        ...
        result.data.symbols = await this.db_fetch_notifysymbols(result.data.symbols)
          .catch((error) => {
            debugError('recv_Plc_Data (cmd: Notification): ' + error);
          });
        this.db_store_symbolhistory(result.data.symbols);
      ...
      case config.ADS_CMD.ReadWrite :
        rxData = await this.track_PlcInvoke_GetRequest(result.header.invoke)
          .catch((error) => {
            debugError('recv_Plc_Data (cmd: ReadWrite): ' + error);
          });
  ...
  return new Promise((resolve, reject) => {
    this.plc_invoke(options, txData, 'uploadinfo')
      .then(async (rxInfo) => {
        ...
        await this.db.symbolinfo_prepare(rxInfo.data.symbols) // db_prepare_symbolinfo(rxInfo.data.symbols);
  ...
  return new Promise((resolve, reject) => {
    this.plc_invoke(options, txData, 'uploadinfo')
      .then(async (rxInfo) => {
        ...
        await this.db.datatypes_prepare(rxInfo.symbols.datatypes);
        resolve(rxInfo.data);
...
BeckhoffClient.prototype.readPlcData = async function(symData) {
  ...
  const options = {
    ...
    symbols : await this.db.symbolhandle_fetch(symData)
  };
...
BeckhoffClient.prototype.callPlcRpcMethod = async function (rpcData) {
  const options = {
    ...
    invoke  : -1,
    request : {},
    //symbols : await this.plc_fetch_symbolhandles(rpcData.symbol)
    symbols : await this.db.symbolhandle_fetch(rpcData.symbol)
  };
  // if no handle found, verify
  // * if rpc method exists
  // + create handle
  // if handle found, use it
