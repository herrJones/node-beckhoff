...
class BeckhoffClient extends events {
  ...
  async plc_fetch_rpcmethods (symbols) {
    const options = {
      cmd     : config.ADS_CMD.INVALID,
      len     : -1,
      invoke  : 0,
      request : {},
      symbols : await this.#db.symbolinfo_fetchrpcmethod(symbols)
    };
  }
 ...
 * 
 * @param {Array} symData info needed per symbol: name
 * @returns {Promise} info on RPC methods
 */
...
BeckhoffClient.prototype.callPlcRpcMethod = async function (rpcData) {
  ...
  //let rpcInfo = [];
  //for (let p = 0; p < rpcData.length; p++) {
  //  rpcInfo.push(rpcData[p].name)
  //}
  //rpcInfo = await this.getRpcMethodInfo();

  const rpcSymbols = []
  for (let p = 0; p < rpcData.length; p++) {
    const newSymbol = rpcData[p].name + '#' + rpcData[p].method

    rpcSymbols.push(newSymbol);
  }
  ...
  const options = {
    ...
    invoke  : -1,
    request : {},
    symbols : await this.plc_fetch_symbolhandles(rpcSymbols)
  };
