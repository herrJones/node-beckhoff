              ...
              this._db
                ...
                .parallelize(() => {
                  datatypes.forEach(element => {
                    ...
                    this.db.run(insStmt, [element.version, element.size, element.offset, element.datatype, element.flags, element.name, element.kind, element.comment, JSON.stringify(element.arrdata), JSON.stringify(element.subdata)], (err) => {
                      if (err) {
...
BeckhoffClient.prototype.setPlcState = async function(stateData) {
  ...
  const options = {
    cmd     : config.ADSCMD.WriteControl,
    len     : 0,
    invoke  : ++this.invokeId,
    request : {},
    symbols : stateData
