...
class BeckhoffClient extends events {
  ...
  create() {
    ...
    //this._plc = new beckhoffBridge(this._settings.plc.ip, this._settings.plc.port);
  ...
  create_database () {
    this._db.serialize(() => {
      this._db.run(`CREATE TABLE IF NOT EXISTS symbols (
        idxGroup       INTEGER,
        idxOffset      INTEGER,
        size           INTEGER,
        name           TEXT,
      ...
      this._db.run(`CREATE TABLE IF NOT EXISTS trx (
        time           REAL,
        kind           TEXT,
        tx             BLOB,
        rx             BLOB   
...
BeckhoffClient.prototype.getPlcInfo = async function(callback) {
  ...
  const txHeader = before.prepareHeader(options, settings);

  try {
    const data = await this._bridge.sendBuffer(txHeader, 'info');
