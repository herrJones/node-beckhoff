                      ...
                      if (err) {
                        ...
                        element.rpccalls.forEach(rpcCall => {
                          this._db.run(rpcStmt, [
                            rpcCall.version, rpcCall.vTableIdx, rpcCall.retSize,
...
BeckhoffClient.prototype.addPlcNotification = async function (symData) {
  ...
  async function newPlcNotification(reference, idx, txData) {
    return new Promise((resolve, reject) => {
      reference.plc_invoke(options, txData, 'addnotify')
        .then(async (rxInfo) => {
          await reference.db_update_symbolhandle(rxInfo.data.symbols[0])
            .catch((error) => {
              debugError(error);
            });
  ...
  return new Promise((resolve, reject) => {
    ...
    //const waitFor = new Int32Array(sab);

    for (let i = 0; i < allSymbols.length; i++) {
      options.invoke = ++this.invokeId;
      options.symbols = new Array(allSymbols[i]);
      txData = before.prepareCommandAddNotification(options, this.settings);

      newPlcNotification(this, notIdx, txData)
        .then((symbol) => {
      ...
      this.plc_invoke(options, txData, 'addnotify')
        .then(async (rxInfo) => {
          await this.db_update_symbolhandle(rxInfo.data.symbols[0])
            .catch((error) => {
              debugError(error);
            });

          allSymbols[i] = rxInfo.data.symbols[0];
          
          // signal the waiting process to continue
          Atomics.store(waitFor, 0, 0);
          Atomics.notify(waitFor, 0, 1);

          if (i+1 == allSymbols.length) {
            resolve(allSymbols);
          }
...
BeckhoffClient.prototype.delPlcNotification = async function (symData) {
  ...
  return new Promise((resolve, reject) => {
    ...
    for (let i = 0; i < allSymbols.length; i++) {
      ...
      // set waitFor[0] = 1 -> flag as busy
      Atomics.store(waitFor, 0, 1);
      ...
      // wait for waitFor[0] to become '0'
      Atomics.wait(waitFor, 0, 0);
    }
    if (allSymbols.length == 0) {
      resolve(allSymbols);
    }
