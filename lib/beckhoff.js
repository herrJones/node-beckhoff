...
class BeckhoffClient extends events {
  ...
  async plc_invoke (options, txBuffer, kind) {
   ...
   /*
    await this._plc.sendBuffer(txBuffer, kind)
      .catch((reason) => {
        throw {
          invoke : options.invoke,
          stage  : 'sendbuffer',
  ...
  db_fetch_symbolhandle (symbols) {
    const result = [];
    let symQry = `select name, handle, notify 
                    from symbols 
                   where (handle <> -1 or notify <> -1)`;
    ...
    return new Promise((resolve, reject) => {
      this._db.all(symQry, symbols, async (err, rows) => {
        if (err) {
          reject(err);
        }
  ...
  /**
   * update the notify handle in the database
   * 
   * @param {Object} symbol object with symbol information
   *
  db_update_notifyhandle (symbol) {
    ...
    return new Promise((resolve, reject) => {
      this._db.run(`
        update symbols 
           set notify = ? where name = ?`, [symbol.notify, symbol.name], (err) => {
        if (err) {
          reject('error updating symbol ' + symbol.name + ' - ' + err.message);
        }
        resolve();
  ...
  */
  ...
  /**
   * release all used symbolhandles
   * 
   * @param {Array} symbols symbols to release. 
   *                If empty, all known symbols wille be released
  ...
  */
  ...
  async destroy() {
    let result = null;
    console.log('releasing notify handles');
    //result = await release notify handles
    
    console.log('releasing symbol handles');
...
BeckhoffClient.prototype.delPlcHandle = async function (symData) {
  let txData = null;
  const symInfo = await this.db_fetch_symbolhandle(symData);
  const options = {
    cmd     : config.ADSCMD.INVALID,
    len     : -1,
    invoke  : ++this.invokeId,
  ...
  return new Promise((resolve, reject) => {
    if (symInfo.length == 1) {
      ...
      const handle = {
        ...
        name      : symInfo[0].name,
        length    : 4,
        handle    : symInfo[0].handle
      };

      options.symbols.push(handle);

      txData = before.prepareCommandWrite(options, this._settings, 'relhandle');
    } else if (symInfo.length  > 1) {
      options.cmd = config.ADSCMD.ReadWrite;

      for (let i = 0; i < symInfo.length; i++) {
        const handle = {
          group  : config.ADSIGRP.RELEASE_SYMHANDLE,
          offset : 0,
          name      : symInfo[i].name,
          length    : 4,
          handle    : symInfo[i].handle
        };
...
BeckhoffClient.prototype.addPlcNotification = async function (symData) {
  ...
  return new Promise((resolve, reject) => {
    ...
    this.plc_invoke(options, txData, 'addnotify')
      .then((rxInfo) => {
        this.db_update_symbolhandle(rxInfo.data.symbols[0]);
