      ...
      this.#db.all(sqlQry, [], (err, rows) => {
        ...
        if (requests[0].hasOwnProperty('mode')) {
          for (let i = 0; i < requests.length; i++) {
            ...
            symbols[idx].delay = requests[i].delay;
            symbols[idx].cycle = requests[i].cycle;
          }
        }
  ...
  //  async recv_Plc_Data (data, db) {
    ...
    switch (result.header.command) {
      ...
      case config.ADSCMD.NotificationDel :
        result.data = after.analyzeDelPlcNotification(data.slice(38));
        break;
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      options = {
        ...
        invoke  : ++this.invokeId,
        request : [{
          idxGroup  : config.ADSIGRP.SYM_DT_UPLOAD,
          idxOffset : 0x00000000,
