...
class BeckhoffClient extends events {
  ...
  async invoke_plc (invokeId, txHeader, kind) {
    let rxInfo = {};
  ...
  async recv_Plc_Data (data, db) {
    ...
    switch (result.header.command) {
      ...
      case config.ADSCMD.Read :
        result.data = after.analyzePlcRead(data.slice(38), null);
        resolved = await this.track_PlcInvoke_Resolve(result.header.invoke, result, db);
        while (resolved != 'OK') {
          await config.sleep(5);
          resolved = await this.track_PlcInvoke_Resolve(result.header.invoke, result, db);
...
BeckhoffClient.prototype.getPlcInfo = async function(callback) {
  ...
  try {
    rxInfo = await this.invoke_plc(options.invoke, txHeader, 'info');
  }
  catch (exc) {
    console.log(exc);
  }
...
BeckhoffClient.prototype.getPlcState = async function(callback) {
  ...
  try {
    rxInfo = await this.invoke_plc(options.invoke, txHeader, 'state');
  }
  catch (exc) {
...
BeckhoffClient.prototype.getPlcSymbols = async function(callback) {
  ...
  try {
    // first command
    rxInfo = await this.invoke_plc(options.invoke, txData, 'read');
    ...
    options = {
      ...
      request : [{
        ...
        idxOffset : 0x00000000,
        length    : Buffer.from(rxInfo.buffer).readUInt32LE(4)
      }]
    }
    //let test = Buffer(rxInfo.buffer).readUInt32LE(4);
    txData = before.preparePlcRead(options, this.settings);
    
    //data = await plc.sendBuffer(txData, 'symbols');
    rxInfo = await this.invoke_plc(options.invoke, txData, 'symbols');

    // clear local database with PLC symbols
    //let symbols = lokiDB.getCollection('symbols');
    //symbols.clear();
const tmp =  Buffer.from(rxInfo.buffer.data);
    //console.log('header : ' + tmp.slice(0, 5).toString('hex') + ':' + tmp.readInt16LE(6));
    //console.log('         ' + tmp.slice(8, 13).toString('hex') + ':' + tmp.readInt16LE(14));
    //console.log('         ' + tmp.readInt16LE(16) + '-' + tmp.readInt16LE(18) + '-' + tmp.readInt32LE(20));
    //console.log('         ' + tmp.readInt32LE(24) + '-' + tmp.readInt32LE(28));

    console.log('data   : ' + tmp.readUInt32LE(0) + '-' + tmp.readInt32LE(4) + '-' + tmp.slice(0, 80).toString());

    // process PLC symbol info
    rxInfo = after.analyzePlcSymbols(Buffer.from(rxInfo.buffer.data), this.db);
