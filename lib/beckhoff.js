    ...
    return new Promise((resolve, reject) => {
      this.#db.symbolinfo_fetch(symbols)
        ...
        .then(async () => {
          ...
          debug('rpcData = ' + JSON.stringify(data));
    ...
    this.db.symbolinfo_fetchrpcmethod(symData)
      .then((rpcData) => {
        ...
        for (let s=0; s < rpcData.length; s++) {
          ...
          //for (let p=0; p < rpcData[s].parm_out.length; p++) {
          //  delete rpcData[s].parm_out[p].kind;
          //  delete rpcData[s].parm_out[p].size;
          //}
        }
        resolve(rpcData);
      })
 ...
 * for the time being: only 1 call at a time
 * 
 * @param {Array} rpcData info needed per symbol: name, method, parms_in
...
BeckhoffClient.prototype.callPlcRpcMethod = async function (rpcData) {
  ...
  return new Promise((resolve, reject) => {
    ...
    let offset = 0;
    for (let p = 0; p <  options.symbols[0].parm_in.length; p++) {
      options.request.buffer.copy(config.createPlcValue(options.symbols[0].parm_in[p], options.symbols[0].parm_in[p].value), offset);
