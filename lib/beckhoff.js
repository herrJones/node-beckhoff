...
class BeckhoffClient extends events {
   ...
   * create and configure (almost) everything
   */
  create() {
  ...
  async plc_fetch_rpcmethodhandles (symbols) {
    ...
    return new Promise((resolve, reject) => {
      this.#db.symbolinfo_fetchrpcmethod(symbols)
        ...
        .then(async (symData) => {
          const data = await this.plc_fetch_symbolhandles(symData, true);
          if (data.length == 0) {
...
BeckhoffClient.prototype.callPlcRpcMethod = async function (rpcData) {
  ...
  return new Promise((resolve, reject) => {
    ...
    let offset = 0;
    for (let p = 1; p <=  options.symbols[0].parm_in.length; p++) {
      const refParm = options.symbols[0].parm_in.find(obj => obj.id == p);
      const reqParm = rpcData[0].parm_in.find(obj => obj.parm === refParm.name);
      
      refParm.value = reqParm.value;
      const value = config.createPlcValue(refParm);
