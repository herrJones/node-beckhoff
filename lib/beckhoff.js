...
function getPlcInfo(callback) {
  ...
  /*
  plc.sendBuffer(txHeader, 'info', 32)
     .then((data) => {
     ...
     .finally(() => {
       plc.sock.destroy();
       callback(rxInfo);//  return data;
     });
  */     
}

function getPlcState(callback) {
  let options = {
  ...
  let plc = new beckhoffClient(settings.plc.ip, settings.plc.port);
  let data = '';
  plc.sendBuffer(txHeader, 'state', 8)
     .then((rxData) => {
       let rxRes = _analyzeHeader(rxData.slice(6, 38));

       data = _analyzePlcState(rxData.slice(38));
...
function getPlcSymbols(callback) {
  //ADSA TX : 00002c000000052a8147010153030a51012f01018980020004000c0000000000000001000000000000000000000000000000
  //ADSA RX : 0000400000000a51012f01018980052a814701015303020005002000000000000000010000000000000018000000b7030000d879010072000000b0f20000d007000000000000
  //ADSA TX : 00002c000000052a8147010153030a51012f01018980020004000c0000000000000002000000000000000000000000000000
  
  //BKHF TX : 00002c000000052a8147010153030a51012f01018980020004000c00000000000000060000000ff000000000000030000000
  //BKHF RX : 0000400000000a51012f01018980052a814701015303020005002000000000000000060000000000000018000000b7030000d879010072000000b0f20000d007000000000000
  let reqData = {
  ...
  let plc = new beckhoffClient(settings.plc.ip, settings.plc.port);
  let data = {};
  plc.sendBuffer(Buffer.concat([txHeader,txData]), 'read', 0x30)
     .then((rxData) => {
       let rxRes = _analyzeHeader(rxData.slice(6, 38));

       data = _analyzePlcRead(rxData.slice(38));

       return data.data.readUInt32LE(4);
     ...
     .then((length) => {
       ...
       return plc.sendBuffer(Buffer.concat([txHeader,txData]), 'symbols', length)
          .then((rxData) => {
             let rxRes = _analyzeHeader(rxData.slice(6, 38));

             let symbols = lokiDB.getCollection('symbols');
             symbols.clear();
             data = _analyzePlcSymbols(rxData.slice(38));
